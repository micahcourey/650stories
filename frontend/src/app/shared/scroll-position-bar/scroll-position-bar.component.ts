...
@Component({
  selector: 'scroll-position-bar',
  templateUrl: './scroll-position-bar.component.html',
  styleUrls: ['./scroll-position-bar.component.scss'],
  host: { 'class': 'ngx-scroll-signal', '[attr.id]': 'id' }
})
export class ScrollPositionBarComponent implements OnInit {

 
  @ViewChild('filler') filler: ElementRef;

  // State
  private _defaultBaseColor = '#ecece8';
  private _defaultFillerColor = '#1cb1d7';
    ...
    this.ngxScrollSignal.windowScroll
      .subscribe((signal: ScrollSignal) =>{
        ...
        // if ( this.isHorizontal ) {
          ...
          this.renderer.setElementStyle(this.filler.nativeElement, 'height', '4px');
